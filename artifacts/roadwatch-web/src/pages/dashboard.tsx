          ...
          <div data-testid="list-weather-alerts">
            {alerts.map((alert: WeatherAlert) => (
              <div className="item" key={alert.id} data-testid={`item-alert-${alert.id}`}>
                <p className="item-title">{alert.event} <span style={{ color: '#a66442', fontWeight: 600, fontSize: 11 }}>· {alert.severity}</span></p>
  ...
  return (
    <div className="app-shell">
      <aside className="rail">
        <a href="/" className="brand" data-testid="link-home" aria-label="Roadwatch home"><span className="brand-mark"><Compass size={21} strokeWidth={1.8} /></span><span><span className="brand-name">roadwatch<span style={{ color: '#b8dcd2' }}>.</span></span><span className="brand-sub">Public works intelligence</span></span></a>
        ...
        <div className="rail-bottom"><div className="eyebrow" style={{ color: '#bfdbd0' }}><span className="live-dot" /> PUBLIC DATA LAYER</div><p>Environmental context for planning, not a road-condition detection system.</p></div>
      </aside>
      <main className="main">
        ...
        <div className="content">
          ...
          <div className="page-heading"><div><h1>Context, not conjecture.</h1><p>Current weather and seismic activity around the places your team watches. Source-reported data only; no road damage is inferred.</p></div><button className="refresh" type="button" disabled={isFetching} onClick={() => void refetch()} data-testid="button-refresh"><RefreshCw size={15} className={isFetching ? 'spinning' : ''} />{isFetching ? 'Refreshing…' : 'Refresh data'}</button></div>
